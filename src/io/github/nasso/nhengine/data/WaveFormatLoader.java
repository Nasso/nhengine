...
public class WaveFormatLoader implements AudioFormatLoader {
	...
	private void readChunk(InputStream in, WaveData data) throws IOException {
		...
		if(chunkID == CHUNKID_FMT_) {
			...
			data.samples = BufferUtils.createByteBuffer(chunkSize);
	...
	public Sound load(String fileName, boolean inJar) throws IOException {
		...
		try(InputStream in = Nhutils.getFileInputStream(fileName, inJar)) {
